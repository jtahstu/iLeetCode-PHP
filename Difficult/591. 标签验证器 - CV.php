...
class Solution
{
    ...
    {
        preg_match('/^(<([A-Z]{1,9})>([^<]|\g<1>|<!\[CDATA\[([^\]]|\](?!\]>))*\]\]>)*<\/\2>)$/', $code, $match);
        return $match;
    }
     ...
     * @param String $code
     * @return Boolean
     * 改方法针对<A><B<![CDATA[asd]]>></B></A>解答是错误的
     */
    function isValid易于理解版($code)
    {
        //验证不合法的标签
        preg_match('/^<([A-Z]{1,9})>(.*)<\/\1>$/', $code, $m);
        if (empty(isset($m[0]) ? $m[0] : '')) {
            return false;
        }
        //把CDATA标签都去掉
        $r = preg_replace("/\<\!\[CDATA\[.*?\]\]\>/", '', $m[2]);
        print_r($r);
        //依次替换正常标签
        while (preg_match('/<([A-Z]{1,9})>([^<]*)?<\/\1>/', $r)) {
            $r = preg_replace('/<([A-Z]{1,9})>([^<]*)?<\/\1>/', '', $r);
        }
...
var_dump((new Solution())->isValid("<A><B<![CDATA[asd]]>></B></A>"));
